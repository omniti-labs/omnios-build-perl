...
AUTHORID=CHRISA
PROG=Devel-DTrace-Provider
MODNAME=Devel::DTrace::Provider
VER=1.11
VERHUMAN=$VER
PKG=omniti/perl/$(echo $PROG | tr '[A-Z]' '[a-z]')
SUMMARY="Create DTrace providers for Perl programs. (Perl $DEPVER)"
DESC="$SUMMARY"

BUILD_DEPENDS_IPS="developer/build/gnu-make system/header system/library/math/header-math omniti/perl/sub-install omniti/perl/sub-exporter"
...
DEPENDS_IPS="omniti/perl/sub-install omniti/perl/sub-exporter"
...
# Hack, since 'make clean' doesn't clean out the libusdt subdir
save_function buildperl32 buildperl32_orig
buildperl32() {
    logmsg "HACK: Cleaning libusdt"
    pushd $TMPDIR/$BUILDDIR/libusdt > /dev/null
    logcmd $MAKE clean
    popd > /dev/null
    buildperl32_orig
}

save_function buildperl64 buildperl64_orig
buildperl64() {
    logmsg "HACK: Cleaning libusdt"
    pushd $TMPDIR/$BUILDDIR/libusdt > /dev/null
    logcmd $MAKE clean
